...
const MAX_FILES = 10; // cap how many files we store to avoid memory issues
let aiState = {
  files: [],
...
export async function aiHandleFileUpload(inputId, statusId) {
  ...
  for (let file of files) {
    if (aiState.files.some(f => f.name === file.name)) {
      skipped.push(`${file.name} (duplicate)`);
      continue;
    }
    if (aiState.files.length >= MAX_FILES) {
      skipped.push(`${file.name} (max files reached)`);
      continue;
    }
...
export async function aiGetAnswer(query, statusId) {
  if (!window.transformers) {
    document.getElementById(statusId).innerText = 'AI library not loaded';
    return { text: "AI features are unavailable. Please check your connection.", sources: [] };
  }
  ...
  try {
    ...
    searchNote = ' (embedding unavailable)';
  }
  let hits = [];
  if (aiState.files.length && embedderModel) {
    try {
      ...
      if (!hits.length && scored.length) searchNote = ' (low similarity to files)';
    } catch (e) {
  ...
  document.getElementById(statusId).innerText = '';
  if (searchNote) answerText += `<br><em>${searchNote}</em>`;
